...
// NOTE: This test requires an available RabbitMQ server at localhost:5672.
func TestAmqp(t *testing.T) {
	// create new broker
	m, err := New("amqp://guest:guest@localhost:5672")
	if err != nil {
	...
	err = r.SendRequest("net", mtype.WalletReq{Net: "net", Type: mtype.ADDRESS, Obj: "0x1234567890", Act: mtype.LISTEN})
	wr := <-req
	if err != nil || wr.Net != "net" || wr.Type != mtype.ADDRESS || wr.Obj != "0x1234567890" || wr.Act != mtype.LISTEN {
