...
func TestExploreChain(t *testing.T) {
	...
	println("Info: running tests against mock blockchain in:", mock.URL)
	...
	bc, err := block.Init([]config.BlockConfig{{Name: net, Node: mock.URL, Secret: "", MaxBlocks: 4}})
	if err != nil {
		t.Errorf("error creating chain:%e", err)
			...
			Addr: []store.Address{
				{
					ID:   []byte{0x31},
					Name: "test",
					Addr: "0x357dd3856d856197c1a000bbAb4aBCB97Dfc92c4",
				},
			},
		},
	}, e.db); err != nil {
	...
	if err != nil {
		t.Errorf("error getting events %e", err)
...
//nolint:gochecknoglobals // test handler
var handler = func(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil || (errors.Is(err, io.EOF) && n == int(r.ContentLength)) {
		...
		res.Error = fmt.Errorf("n:%d error:%w", n, err)

		return
	}
	// unmarshal JSON body
	if err = json.Unmarshal(body, &req); err != nil {
		res.Error = fmt.Errorf("Error unmarshaling Body:%w", err)
...
func TestManageWalletRequest(t *testing.T) {
	...
	// define a chain
	bc, err := block.Init([]config.BlockConfig{{Name: net, Node: "https://localhost:8545", Secret: "", MaxBlocks: 4}})
	if err != nil {
		t.Errorf("cannot init blockchain %e", err)
	...
	// test steps: a WalletReq is sent. Then check the map's len, and if an object is specified, we check the
	// existence of that object in the map
	ts := []interface{}{
		[]interface{}{msg.WalletReq{Net: net, Type: msg.ADDRESS, Obj: "addr1", Act: msg.UNLISTEN}, 0, ""},
		[]interface{}{msg.WalletReq{Net: net, Type: msg.ADDRESS, Obj: "addr1", Act: msg.LISTEN}, 1, "addr1", true},
		[]interface{}{msg.WalletReq{Net: net, Type: msg.ADDRESS, Obj: "addr2", Act: msg.LISTEN}, 2, "addr2", true},
		[]interface{}{msg.WalletReq{Net: net, Type: msg.ADDRESS, Obj: "addr1", Act: msg.LISTEN}, 2, "addr1", true},
		[]interface{}{msg.WalletReq{Net: net, Type: msg.ADDRESS, Obj: "addr1", Act: msg.UNLISTEN}, 1, "addr1", false},
	}

	// run test
	for _, step := range ts {
		// send request
		if err = mb.SendRequest(net, step.([]interface{})[0].(msg.WalletReq)); err != nil {
